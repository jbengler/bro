...
The basic layout of the heatmap relies on the parameters `rows`, `columns` and `values`. You can think of them like aesthetics in `ggplot2::ggplot()`, similar to somthing like `aes(x = columns, y = rows, fill = values)`.
...
Rows and columns in the heatmap will appear in the same order as in the tidy data frame useed as input. For example, to order rows and columns alphabetically, just use the `dplyr::arrange()`.
...
Gaps can be added by specifying data frame variables that should be used to generate the gaps. Only one variable can be choosen for `gaps_row` and one for `gaps_col`.
